...
public class Minitor_overall {
    ...
    private Set<Minitor_group> groups = new HashSet<Minitor_group>();

    @OneToMany(mappedBy = "overall")
    @LazyCollection(LazyCollectionOption.EXTRA)
    @Cascade(value = {CascadeType.ALL})
    private Set<Minitor_resource> resources = new HashSet<Minitor_resource>();

    @OneToMany(mappedBy = "overall")
    @LazyCollection(LazyCollectionOption.EXTRA)
    @Cascade(value = {CascadeType.ALL})
    private Set<Minitor_method> methods = new HashSet<Minitor_method>();

    @OneToMany(mappedBy = "overall")
    @LazyCollection(LazyCollectionOption.EXTRA)
    @Cascade(value = {CascadeType.ALL})
    private Set<Minitor_services> services = new HashSet<Minitor_services>();

    public Minitor_overall() {
    }
    ...
    public Set<Minitor_group> getGroups() {
        return groups;
    }

    public void setGroups(Set<Minitor_group> groups) {
        this.groups = groups;
    }

    public Set<Minitor_resource> getResources() {
        return resources;
    }

    public void setResources(Set<Minitor_resource> resources) {
        this.resources = resources;
    }

    public Set<Minitor_method> getMethods() {
        return methods;
    }

    public void setMethods(Set<Minitor_method> methods) {
        this.methods = methods;
    }

    public Set<Minitor_services> getServices() {
        return services;
    }

    public void setServices(Set<Minitor_services> services) {
